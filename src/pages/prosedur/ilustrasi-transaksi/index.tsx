...
export default function IlustrasiTransaksi() {
  ...
  const keteranganLines = t("keteranganLines", { returnObjects: true }) as string[];
  const forexTable = t("forexTable", { returnObjects: true }) as Array<{
    kode: string;
    dasar: string;
    kategori: string;
    tipe: string;
      ...
      <div className="px-4 sm:px-8 md:px-12 lg:px-20 xl:px-52 my-10">
        <ProfilContainer title={t("title")}>
          <div className="prose max-w-none text-gray-700">
            ...
            <h1 className="text-3xl font-bold mb-6 border-b pb-3">{t("contentTitle")}</h1>

            {/* Rumus */}
            <section className="mt-10">
              <h2 className="text-xl font-semibold mb-2">{t("formulaSectionTitle")}</h2>
            ...
            {/* Keterangan */}
            <section className="mt-10">
              <h2 className="text-xl font-semibold mb-3">{t("keteranganTitle")}</h2>
            ...
            <section className="mt-12">
              ...
              {[t("ex1.f1"), t("ex1.f2"), t("ex1.f3")].map((f, i) => (
                <pre key={i} className="bg-gray-50 border border-gray-200 p-3 rounded text-sm whitespace-pre-wrap mt-2">
                  {f}
                </pre>
              ))}
            ...
            <section className="mt-12">
              ...
              {[t("ex2.f1"), t("ex2.f2"), t("ex2.f3"), t("ex2.f4")].map((f, i) => (
                <pre key={i} className="bg-gray-50 border border-gray-200 p-3 rounded text-sm whitespace-pre-wrap mt-2">
                  {f}
                </pre>
              ))}
            ...
            <section className="mt-16">
              <h2 className="text-2xl font-semibold mb-2 border-b pb-2">{t("forex.sectionTitle")}</h2>

              <h3 className="font-semibold mt-4">{t("forex.tableTitle")}</h3>
                  ...
                  <thead className="bg-gray-100 text-gray-700">
                    <tr>
                      {["code", "base", "category", "type"].map((h) => (
                        <th key={h} className="border px-3 py-2 font-medium">
                          {t(`forex.headers.${h}`)}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {forexTable.map((row, idx) => (
                      <tr key={idx} className="odd:bg-white even:bg-gray-50">
                        ...
                        <td className="border px-3 py-2">{row.kategori}</td>
              ...
              <h3 className="font-semibold mt-6">{t("forex.formulaTitle")}</h3>
              <p className="mt-2"><span className="font-semibold">{t("forex.directRatesTitle")}:</span> {t("forex.directFormula")}</p>
              <p><span className="font-semibold">{t("forex.indirectRatesTitle")}:</span> {t("forex.indirectFormula")}</p>

              {/* Contoh Forex */}
              <h4 className="font-semibold mt-10">{t("forex.examplesTitle")}</h4>

              {/* Example 1 */}
              <h4 className="font-semibold mt-2">{t("forex.ex1.title")}</h4>
              <p>{t("forex.ex1.p1")}</p>
              {[t("forex.ex1.f1"), t("forex.ex1.f2"), t("forex.ex1.f3"), t("forex.ex1.f4")].map((f, i) => (
                <pre key={i} className="bg-gray-50 border border-gray-200 p-3 rounded text-sm whitespace-pre-wrap mt-2">
                  {f}
                </pre>
              ))}
              ...
              {/* Example 2 */}
              <h4 className="font-semibold mt-6">{t("forex.ex2.title")}</h4>
              <p>{t("forex.ex2.p1")}</p>
              ...
              <p className="mt-4 font-semibold">{t("forex.ex2.altHeading")}</p>
              {[t("forex.ex2.af1"), t("forex.ex2.af2"), t("forex.ex2.af3")].map((f, i) => (
                <pre key={i} className="bg-gray-50 border border-gray-200 p-3 rounded text-sm whitespace-pre-wrap mt-2">
                  {f}
                </pre>
              ))}
